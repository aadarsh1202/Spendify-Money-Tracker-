...
class viewtransactionsbymonth(Resource):
    @cross_origin()
    def post(self):
        try:
            DB_NAME = os.getenv('FIRESTORE_TRANSACTIONS_TABLE_NAME')
            ...
            for date in dates:
                ...
                if month == date[5:7] and year == date[0:4]:
                    monthData[date] = transactions_dict[date]   

            # if date in transactions_dict:
            #     return jsonify({'message': 'date found','data':transactions_dict[date]})
            # else:
            if(monthData):
                return jsonify({'message': 'data found','data':monthData})
            else:
                return jsonify({'message': 'data not found'})
...
class viewtransactionsbyyear(Resource):
    @cross_origin()
    def post(self):
        try:
            DB_NAME = os.getenv('FIRESTORE_TRANSACTIONS_TABLE_NAME')
            ...
            for date in dates:
                ...
                # month = date[5:7]
                if year == date[0:4]:
                    yearData[date] = transactions_dict[date]   

            if(yearData):
                return jsonify({'message': 'data found','data':yearData})
            else:
                return jsonify({'message': 'data not found'}) 
